...
cv.bin.strat.class<-function(dataset, seed=123456, cv=10){
  ...
  balance<-table(dataset$Reported_Severity)  
  ...
  for(cv.i in 1:(cv-1)){
    tmp<-which(dataset$Reported_Severity[actual.inst]==(names(balance)[1]))
    c1<-actual.inst[sample(tmp, n.inst.c0)]
    tmp<-which(dataset$Reported_Severity[actual.inst]==(names(balance)[2]))
