...
# They are at the folder results/final_models/

library(gplots)

# Load and clean the prediction files
nb = read.table("results/final_models/naive.bayes.csv", header=T, sep=",", quote="\"", stringsAsFactors = F)
dt = read.table("results/final_models/decision.tree.csv", header=T, sep=",", quote="\"", stringsAsFactors = F)

# Remove instances from the training set (443 instances)

training = read.table("dataset/HemB_Dataset_v5a.csv", header=T, sep="\t", quote="", stringsAsFactors = F)
